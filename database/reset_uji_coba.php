...
// nempel ikut kehapus. Tabel notifikasi juga DIKOSONGIN TOTAL tiap akun
// (2026-09-07, diminta user) - beda dari cuti_pegawai, notifikasi gak
// punya baris "asli" yang perlu dipertahankan, jadi langsung DELETE semua.
//
...
// Notifikasi tiap akun ikut dikosongin - semuanya hasil pengajuan/approval
// uji coba, gak ada notifikasi "asli" yang perlu dipertahankan (beda dari
// cuti_pegawai yang punya baris Cuti Historis asli di atas).
$jumlahNotif = (int) db_one($db, "SELECT COUNT(*) AS n FROM notifikasi")['n'];
db_query($db, "DELETE FROM notifikasi");
...
echo "[" . date('c') . "] Reset uji coba selesai: " . (count($rows) - $historisDilewati) . " baris cuti_pegawai dihapus, "
    ...
    . "$jumlahNotif notifikasi dihapus, "
